...
#:
#: 687 へ上げたのは、issue #23（`argv[0]` の解決を呼び出し元の PATH に合わせる）の
#: 修正で、Wrapper Spec に解決の規則（fail-open の条件）と、起動できた実体を
#: ログの見出しへ残す仕様を 1 段落足したためである。今後も真であり続ける仕様であって
#: 経緯ではない。
#:
#: 690 へ上げたのは、issue #13 第 1 段（フックの文言を日英併記にする）で、
#: Hook Spec に言語判定の順序（環境変数 → Claude Code の設定 → OS のロケール →
#: 既定は日本語）を 3 行足したためである。今後も真であり続ける仕様であって経緯ではない。
#:
#: 699 へ上げたのは、issue #13 第 2 段（CLI の文言を日英併記にする）で、Hook Spec に
